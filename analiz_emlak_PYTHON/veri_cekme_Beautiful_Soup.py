...
for i in range(1, 256):  # Change range as needed
    adres = f'https://www.hepsiemlak.com/antalya-satilik/daire?counties=antalya-aksu,dosemealti,kepez,konyaalti,muratpasa&page={i}'
    ...
    baslik = {'User-Agent': random.choice(user_agents)}

    try:
        sayfa = requests.get(adres, headers=baslik)
        sayfa.raise_for_status()  # Raise an error for bad responses
        soup = bs(sayfa.content, 'lxml')

        fiyatlar = soup.find_all('span', {'class': 'list-view-price'})
        m_2 = soup.find_all('span', {'class': 'celly squareMeter list-view-size'})
        oda_sayisi_elements = soup.find_all('span', {'class': 'celly houseRoomCount'})
        bina_yasi_elements = soup.find_all('span', {'class': 'celly buildingAge'})
        bulundugu_kat_elements = soup.find_all('span', {'class': 'celly floortype'})
        mahalle_elements = soup.find_all('span', class_='list-view-location')

        time.sleep(random.randint(3, 30))

        for i in range(len(fiyatlar)):
            # Parse and append data (similar to your existing code)
            try:
                # Example for fiyat parsing
                fiyat.append(int(fiyatlar[i].text.strip().replace('.', '').replace('TL', '')))
                m_2a.append(int(m_2[i].text.strip().replace(' m²', '')))
                oda_sayisi.append(oda_sayisi_elements[i].text.strip())
                bina_yasi.append((''.join(filter(str.isdigit, bina_yasi_elements[i].text.strip()))))
                bulundugu_kat.append((''.join(filter(str.isdigit, bulundugu_kat_elements[i].text.strip()))))
...
data.to_excel("antalya_muratpasa2.xlsx", index=False)
